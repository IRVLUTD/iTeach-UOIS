...
class HumanPlayDataset(data.Dataset, datasets.imdb):

    def __init__(self, image_set="train", humanplay_object_path = None, eval=False):

        self._name = 'humanplay_object_' + image_set
        self._image_set = image_set
        self._humanplay_object_path = self._get_default_path() if humanplay_object_path is None \
                            else humanplay_object_path
        ...
        if image_set == 'train':
            data_path = os.path.join(self._humanplay_object_path, 'training_set')
            self._humanplay_object_path = data_path
        elif image_set == 'test':
            data_path = os.path.join(self._humanplay_object_path, 'test_set')
        ...
        assert os.path.exists(self._humanplay_object_path), \
                'humanplay_object path does not exist: {}'.format(self._humanplay_object_path)
    
    def get_my_intrinsic_matrix(self):
        """
        Source: 
    ...
    def list_dataset(self):
        data_path = Path(self._humanplay_object_path)
        # print('data path', data_path)
        seqs = sorted(list(Path(data_path).glob('scene*')))
    ...
    def __getitem__(self, idx):
        ...
        if cfg.INPUT == 'DEPTH' or cfg.INPUT == 'RGBD':
            # Depth image
            depth_img_filename = filename.replace('rgb', 'depth')
            ...
            intrinsics = self.get_my_intrinsic_matrix()
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(datasets.ROOT_DIR, 'data', 'humanplay_data')
