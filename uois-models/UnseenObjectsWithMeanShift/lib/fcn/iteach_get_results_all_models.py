...
if __name__ == "__main__":
    ...
    weight_path_MSMFormer = os.path.join(
        dirname, "../../data/rgbd_pretrain/norm_RGBD_pretrained.pth"
    )

    # Finetuned model from 0
    cfg_file_MSMFormer = os.path.join(
        dirname, f"../../MSMFormer/{sys.argv[1]}/config.yaml"
    )
    weight_path_MSMFormer = os.path.join(
        dirname, f"../../MSMFormer/{sys.argv[1]}/model_final.pth"
    )
    ...
    # directory = "../../MSMFormer/human_play_rgbd_f2_mix" # train-0
    # directory = "../../MSMFormer/human_play_rgbd_f2_mix_using_pretrained_20K_250"
    # directory = "../../MSMFormer/human_play_rgbd_f2_mix_2120_250" # ft
    # directory = "../../MSMFormer/human_play_rgbd_f2_mix_20K_250"

    for directory in glob.glob("../../MSMFormer/scene_ablation_human_play_rgbd_f2_mix_s*"):

        model_results = f"{directory}/model_results"

        if "scene_ablation_human_play_rgbd_f2_mix_s3/" in model_results:
            print(f"skipped: {model_results}")
            continue
        ...
        for pth_file in tqdm(pth_files_sorted):
            try:
                cfg_file_MSMFormer = f"{directory}/config.yaml"
                weight_path_MSMFormer = pth_file

                _ = weight_path_MSMFormer.split('/') # ../../MSMFormer/human_play_rgb_f2_mix/model_0000249.pth
                exp, pth = _[-2], _[-1]
                
                logging.info(weight_path_MSMFormer)
                ...
                print(pth_results.keys())

                json_path = os.path.join(model_results, f"{pth}.json")

                with open(json_path, "w") as f:
                    json.dump(pth_results, f, indent=4)
