...
"""
iTeach MaskFormer Training Script.
...
class Trainer(DefaultTrainer):
    ...
    def build_model(cls, cfg):
        ...
        if cfg.USE_LoRA:
            ...
            target_modules = [
                name for name, module in model.pretrained_backbone.named_modules()
                if isinstance(module, supported_types) and "stem" not in name  # Exclude "stem"
            ]
            # """
            # LoRA configuration
            lora_rank = 8
            ...
            lora_config = LoraConfig(
                ...
                lora_dropout=0.15,
                # target_modules=["res5"],  # Apply LoRA to the res5 block
                target_modules=target_modules
            )
            
            # Apply LoRA to the entire backbone
            model.pretrained_backbone = get_peft_model(model.pretrained_backbone, lora_config)
            model.pretrained_backbone.print_trainable_parameters()
...
def setup(args):
    ...
    # cfg.INPUT.INPUT_IMAGE = "" # don't uncomment unless wanna override personally
...
def custom_argument_parser():
    ...
    parser.add_argument("--use_lora", action="store_true", help="use lora layers to finetune")  
